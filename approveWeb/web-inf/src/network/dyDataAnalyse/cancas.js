 export function drawDiagonal(data,loopnameb,number){
  ...
  //判断 相别有几种 3 6
   ...
   //数据格式:["0321,0456-0567,0678","0422,0223-0588,0666"]0321,0456 - 0567,0678      >0321代表每一片的值,0321,0456代表A1串0567,0678代表A2串
   //每个相限存一个对应的字符串,不同的串中间用-来分隔开
      ...
      for (var j = 0; j < resganta[i].length; j++) {
        ...
        for (var k = 0; k < resganta[i][j].length; k++) {//遍历每串的个数
          if (i==0) {//A相上
            ...
            context.stroke();
            if (k == 0) {
              if (resganta[i][j][k] <= resganta[i][j][k+1]/2) {
            ...
            }else if (k == resganta[i][j].length-1) {
              if (resganta[i][j][k] <= resganta[i][j][k-1]/2) {
                EllipseOne(context,500-x,135+y,30,3);
                y+=10;
                context.fillStyle = "#d62323"; //故障绝缘子颜色
              ...
              if (resganta[i][j][k] <= resganta[i][j][k+1]/2 && resganta[i][j][k] <= resganta[i][j][k-1]/2 ) {
                EllipseOne(context,500-x,135+y,30,3);
                y+=10;
                context.fillStyle = "#d62323"; //故障绝缘子颜色
                context.fill();//填充
