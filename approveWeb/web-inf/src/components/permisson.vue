...
<!--    <el-input v-model="filterText" placeholder="Filter keyword" style="margin-bottom:30px;"/>-->
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            size="mini"
            @click="() => {dialogFormVisible = true, menu.pid = scope.row.id,dialogFormValue='添加菜单'}">添加菜单
          </el-button>
          <el-button
            v-if="scope.row.level == 3"
            type="text"
            size="mini"
            @click="() => {dialogPermissionVisible = true, permission.pid = scope.row.id,diglogvalue='添加功能'}">添加功能
          </el-button>
          <el-button
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="dialogFormValue" @close="restData()">
    ...
    <!-- 添加功能的窗口 -->
    <el-dialog :visible.sync="dialogPermissionVisible" :title="diglogvalue" @close="restData()">
...
export default {
  ...
  methods: {
    fetchNodeList() {
      menu.getNestedTreeList().then(response => {
       // console.log(response.data)
        this.menuList = response.data.data.children
    ...
    getById(data) {
      this.dialogFormValue="修改菜單"
      this.dialogFormVisible = true
      this.menu = data
      this.menu = JSON.parse(JSON.stringify(data));
    },
    updateFunction(data) {
      this.diglogvalue="修改功能"
